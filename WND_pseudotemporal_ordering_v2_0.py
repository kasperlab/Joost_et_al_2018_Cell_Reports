################################################################################
################## STRT-WOUND -- PSEUDOTEMPORAL ORDERING ###################
################################################################################
...
def fit_vgam_v1(dataset, PTO_coords, genes, df, neg_log = False):
    ...
    stats_ = pd.DataFrame(index = genes, columns = ['Chisq','Pr(>Chisq)','Pr(>Chisq)-BH'])
    
    #run
    
    for g in genes:
        
        tmp = fit_vgam_v1P_helper(dataset.ix[g], PTO_coords, g, df)
        fitted.ix[g] = tmp[0]
        stats_.ix[g, 'Chisq'] = tmp[1]
        stats_.ix[g, 'Pr(>Chisq)'] = tmp[2]
        
    stats_['Pr(>Chisq)-BH'] = stats.p_adjust(FloatVector(stats_['Pr(>Chisq)']), method = 'BH')
    
    if neg_log == True:
        stats_['Pr(>Chisq)'] = -np.log10(stats_['Pr(>Chisq)'])
        stats_['Pr(>Chisq)-BH'] = -np.log10(stats_['Pr(>Chisq)-BH'])
        
    return fitted, stats_
...
def PTO_draw_MST_groups(MST, MST_pos, diam_edges, sample_groups, cmap = plt.cm.jet, node_size = 100, 
    ...
    if type(cmap) != dict:
        cm = cmap
        cmap = {}
        for ix, gr in enumerate(return_unique(sample_groups)):
            cmap[gr] = cm(float(ix) / len(set(sample_groups)))
    
    clist = [cmap[sample_groups[node]] for node in MST.nodes()]
...
def PTO_plot_gene(dataset, PTO_coords, sample_groups, gene, fit = None, cmap = plt.cm.jet):
    ...
    clist = [cmap[gr] for gr in sample_groups[PTO_coords.index].values]
...
def PTO_correlate_find_max(corr_r, return_r = False):
    ...
    for ix in corr_r.index:
        ...
        corr_max[ix] = float(corr_r.ix[ix].sort_values().index[-1])
        corr_max_r[ix] = corr_r.ix[ix].sort_values()[-1]
...
def PTO_order_groups_InPe(gene_groups, in_pe_de, sort = ['induction','peak','deactivation']):
    ...
    for gr in return_unique(gene_groups):
        ...
        if sort == 'middle':
            ...
            sort_tmp = list(in_pe_de_mid.ix[ix_tmp].sort_values().index)
