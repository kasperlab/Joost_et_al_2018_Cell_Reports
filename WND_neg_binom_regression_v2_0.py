################################################################################
########## STRT-WOUND -- BAYESIAN NEGATIVE BINOMINAL REGRESSION ############
################################################################################
...
def neg_binom_vs_groups_v2P(summary, dview, PP='99.9', consider_Baseline = False):
    ...
    def neg_binom_test_vs_groups(gene, summary, PP, consider_Baseline):
        ...
        if consider_Baseline == True:
            first, second = summary.ix['mean'].order().index[-1], summary.ix['mean'].order().index[-2]
            
        else:
            first, second = summary.ix['mean', [gr for gr in summary.columns if gr != 'Baseline']].order().index[-1], summary.ix['mean', [gr for gr in summary.columns if gr != 'Baseline']].order().index[-2]
...
def neg_binom_extract_genes_v2(distances, score, number = 10):
    
    dist = distances[score]
    
    output = pd.DataFrame(index = range(number), columns = dist.columns)
    
    for gr in dist.columns:
        ordered = [g for g in dist[gr].sort_values()[::-1].index if dist.ix[g, gr] > 0.0][:number]
        output.ix[range(len(ordered)), gr] = ordered
        
    return output

################################################################################

def neg_binom_get_fold_change(summary, binarized, reference):
    ...
    for gr in binarized.columns:
        ...
        for g in g_sel:
            output_mean.ix[g,gr] = np.log2(summary.ix[g].ix['mean'][gr]) - np.log2(summary.ix[g].ix['mean'][reference])
            output_median.ix[g,gr] = np.log2(summary.ix[g].ix['median'][gr]) - np.log2(summary.ix[g].ix['median'][reference])
            output_P5.ix[g,gr] = np.log2(summary.ix[g].ix['Q5'][gr]) - np.log2(summary.ix[g].ix['Q95'][reference])
            output_P25.ix[g,gr] = np.log2(summary.ix[g].ix['Q25'][gr]) - np.log2(summary.ix[g].ix['Q75'][reference])
...
def neg_binom_estimate_prob_all_groups_v1P(traces, dview, repeats = 100000):
    
    
    def neg_binom_estimate_prob_all_groups_helper_overexpr_v1(traces, gene, repeats = repeats):
        ...
        output = pd.DataFrame(index = [gene], columns = traces.index).fillna(0)
        
        #iterate over groups
        
        for ref in traces.index:
            pval_tmp = neg_binom_estimate_prob_helper_overexpr_v1(traces, gene, ref, repeats)
            
        #replace with highest values
        
            for gr in pval_tmp.columns:
                if pval_tmp.ix[gene, gr] > output.ix[gene, gr]:
                    output.ix[gene, gr] = pval_tmp.ix[gene, gr]
    ...
    output_tmp = dview.map_sync(neg_binom_estimate_prob_all_groups_helper_overexpr_v1,
                                [traces.ix[g] for g in traces.index],
                                [g for g in traces.index],
                                [repeats] * l)
